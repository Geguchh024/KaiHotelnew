        ...
        <main className="pt-[72px] min-h-screen bg-background flex items-center justify-center">
          <div className="text-center py-14 px-6">
            ...
            <h2 className="font-[EB_Garamond] text-[22px] text-primary mb-2 font-georgian">
              {locale === 'ka' ? 'რეზერვაცია ვერ მოიძებნა' : locale === 'ru' ? 'Бронирование не найдено' : 'Reservation not found'}
            </h2>
            <p className="font-[Hanken_Grotesk] text-[12px] text-secondary mb-6 font-georgian">
              {locale === 'ka' ? 'მითითებული კოდით რეზერვაცია არ არსებობს.' : locale === 'ru' ? 'Бронирования с указанным кодом не существует.' : 'No reservation exists with the provided reference code.'}
            </p>
        ...
        <div className="max-w-[720px] mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5 sm:space-y-6">
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/20 p-4 sm:p-5 text-center">
            <p className="font-[Hanken_Grotesk] text-[10px] font-semibold uppercase tracking-[0.05em] text-on-surface-variant mb-1 font-georgian">
              {locale === 'ka' ? 'საცნობარო კოდი' : locale === 'ru' ? 'Справочный код' : 'Reference Code'}
            </p>
            ...
            {isGroup && (
              <p className="font-[Hanken_Grotesk] text-[10px] text-on-surface-variant mt-2 font-georgian">
                {locale === 'ka'
                  ...
                  : locale === 'ru'
                  ? `Групповое бронирование — ${groupItems.length} номеров забронированы вместе`
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/20 p-4 sm:p-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2 text-[11px]">
                  <div className="flex justify-between">
                    <span className="text-secondary font-georgian">{locale === 'ka' ? 'სრული სახელი' : locale === 'ru' ? 'Полное имя' : 'Full Name'}</span>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/20 p-4 flex items-center justify-between">
            <span className="font-[Hanken_Grotesk] text-[11px] text-secondary font-georgian">
              {locale === 'ka' ? 'სტატუსი' : locale === 'ru' ? 'Статус' : 'Status'}
            </span>
...
function StatusPill({ status, locale }: { status: string; locale: Locale }) {
  const labels: Record<string, { ka: string; en: string; ru: string }> = {
    pending: { ka: 'მოლოდინში', en: 'Pending', ru: 'В ожидании' },
    confirmed: { ka: 'დადასტურებული', en: 'Confirmed', ru: 'Подтверждено' },
    checkedIn: { ka: 'შესული', en: 'Checked In', ru: 'Заселен' },
    checkedOut: { ka: 'გასული', en: 'Checked Out', ru: 'Выселен' },
    cancelled: { ka: 'გაუქმებული', en: 'Cancelled', ru: 'Отменено' },
    noShow: { ka: 'არ გამოცხადდა', en: 'No Show', ru: 'Неявка' },
  }
